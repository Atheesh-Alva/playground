...
class SchemaParser {
  static List<Map<String, dynamic>> fetchAppSyncApis({
    ...
    final String mutationType = schema["data"]["__schema"]["mutationType"]["name"];
    final String subscriptionType = schema["data"]["__schema"]["subscriptionType"]["name"];
    ...
    final List<dynamic> types =
        allTypes.where((e) => (e['name'] != queryType) && (e['name'] != mutationType) && (e['name'] != subscriptionType)).toList();
    for (dynamic item in queries + mutations + subscriptions) {
      final String responseType = item["type"]["name"] ?? item["type"]["ofType"]["name"];
      String inputDef = '', inputConts = '', outputDef = '';

      List<dynamic> inputs = [], outputs = [];
      for (var item in item["args"]) {
        String dataType = (item["type"]["name"] ?? item["type"]["ofType"]["name"]) + (item["type"]["kind"] == "NON_NULL" ? "!" : "");
        inputs.add({"dataType": dataType, "fieldName": item["name"]});
        inputDef = inputDef.isEmpty ? '\$${item["name"]}: $dataType' : '$inputDef, \$${item["name"]}: $dataType';
        inputConts = inputConts.isEmpty ? '${item["name"]}: \$${item["name"]}' : '$inputConts, ${item["name"]}: \$${item["name"]}';
      }
      ...
      for (var item in responseFileds) {
        ...
        outputDef = outputDef.isEmpty ? '${item["name"]}' : '$outputDef ${item["name"]}';
      }
      ...
      apis.add({
        ...
        "api": '${apiName.toLowerCase()} ${item["name"]}($inputDef) {${item["name"]}($inputConts) {$outputDef}}'
      });
    }
    return apis;
  }
  ...
  //     String dataType = (input["type"]["name"] ?? input["type"]["ofType"]["name"]) + (input["type"]["kind"] == "NON_NULL" ? "!" : "");
  //     inputParameterDef = inputParameterDef.isEmpty ? '\$${input["name"]}: $dataType' : '$inputParameterDef, \$${input["name"]}: $dataType';
  //     constructor = constructor.isEmpty ? '${input["name"]}: \$${input["name"]}' : '$constructor, ${input["name"]}: \$${input["name"]}';
  //   }
  //   for (dynamic item in apiTypes) {
  //     if (item['name'] == (output["name"] ?? output["ofType"]["name"])) {
  //       for (dynamic field in item['fields']) {
  //         outputs = outputs.isEmpty ? '${field["name"]}' : '$outputs ${field["name"]}';
  //       }
  //     }
  //   }
